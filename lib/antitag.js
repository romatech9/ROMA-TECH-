// ============================================================
// MUFASER-X — ANTITAG — DELETE ANY MENTION INCLUDING @roma
// ============================================================
...
function isTagMessage(msg){
  ...
  const mentions = m.extendedTextMessage?.contextInfo?.mentionedJid || m.imageMessage?.contextInfo?.mentionedJid || m.videoMessage?.contextInfo?.mentionedJid || [];
  const groupMentions = m.extendedTextMessage?.contextInfo?.groupMentions || [];

  // NOW DETECTS SINGLE TAG TOO
...
async function handleAntiTag(sock, msg, account){
  try{
    ...
    if(settings.mode==='warn'){
      await sock.sendMessage(jid,{text:`📢 @${senderNumber} Tagging/Mentioning is not allowed!`,mentions:[senderJid]});
    }
    return true;
  }catch(e){ console.error('[AntiTag Error]',e.message); return false; }
}

module.exports={handleAntiTag, getSettings};