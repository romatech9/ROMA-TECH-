module.exports={
  name:'antisong',
  desc:'Block songs in group',
  category:'Group',
  usage:'.antisong delete on / warn on / off',
  async execute(sock,msg,jid,args,sender,account){
    try{
      ...
      if(!isBotAdmin) return sock.sendMessage(jid,{text:'❌ *I need to be a group admin to use the antisong command.*'},{quoted:msg});
      if(!account.antisong) account.antisong={}; if(!account.antisong[jid]) account.antisong[jid]={mode:'off'};
      const s=account.antisong[jid]; const type=String(args[0]||'').toLowerCase(); const action=String(args[1]||'').toLowerCase();
      if(type==='delete'&&action==='on'){ s.mode='delete'; return sock.sendMessage(jid,{text:'🚫 *ANTISONG ENABLED*\n🗑️ Songs will be deleted.'},{quoted:msg}); }
      if(type==='warn'&&action==='on'){ s.mode='warn'; return sock.sendMessage(jid,{text:'⚠️ *ANTISONG WARN ENABLED*\n⚠️ Will delete + warn, no kick.'},{quoted:msg}); }
      if(type==='off'||action==='off'){ s.mode='off'; return sock.sendMessage(jid,{text:'✅ *ANTISONG DISABLED*'},{quoted:msg}); }
      if(type==='on'){ s.mode='delete'; return sock.sendMessage(jid,{text:'🚫 *ANTISONG ENABLED*\n🗑️ Songs will be deleted.'},{quoted:msg}); }
      return sock.sendMessage(jid,{text:'🎵 *ANTISONG*\n\n`.antisong delete on` → just delete\n`.antisong warn on` → delete + warn (no kick)\n`.antisong off` → disable'},{quoted:msg});
