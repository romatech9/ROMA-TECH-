module.exports={
  name:'antiforward',
  desc:'Block forwarded messages in group',
  category:'Group',
  usage:'.antiforward delete on / warn on / off',
  async execute(sock,msg,jid,args,sender,account){
    try{
      ...
      if(!isBotAdmin) return sock.sendMessage(jid,{text:'❌ *I need to be a group admin to use the antiforward command.*'},{quoted:msg});

      if(!account.antiforward) account.antiforward={}; if(!account.antiforward[jid]) account.antiforward[jid]={mode:'off'};
      const s=account.antiforward[jid];
      const type=String(args[0]||'').toLowerCase(); const action=String(args[1]||'').toLowerCase();

      if(type==='delete'&&action==='on'){ s.mode='delete'; return sock.sendMessage(jid,{text:'🚫 *ANTIFORWARD ENABLED*\n🗑️ Forwarded messages will be deleted.'},{quoted:msg}); }
      if(type==='warn'&&action==='on'){ s.mode='warn'; return sock.sendMessage(jid,{text:'⚠️ *ANTIFORWARD WARN ENABLED*\n⚠️ Will delete + warn, no kick.'},{quoted:msg}); }
      if(type==='off'||action==='off'){ s.mode='off'; return sock.sendMessage(jid,{text:'✅ *ANTIFORWARD DISABLED*'},{quoted:msg}); }
      if(type==='on'){ s.mode='delete'; return sock.sendMessage(jid,{text:'🚫 *ANTIFORWARD ENABLED*\n🗑️ Forwarded messages will be deleted.'},{quoted:msg}); }

      return sock.sendMessage(jid,{text:'↗️ *ANTIFORWARD*\n\n`.antiforward delete on` → just delete\n`.antiforward warn on` → delete + warn (no kick)\n`.antiforward off` → disable'},{quoted:msg});
