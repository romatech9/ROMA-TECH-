module.exports={
  name:'antitext',
  desc:'Block text messages in group',
  category:'Group',
  usage:'.antitext delete on / warn on / off',
  async execute(sock,msg,jid,args,sender,account){
    try{
      ...
      if(!isBotAdmin) return sock.sendMessage(jid,{text:'❌ *I need to be a group admin to use the antitext command.*'},{quoted:msg});

      if(!account.antitext) account.antitext={}; if(!account.antitext[jid]) account.antitext[jid]={mode:'off'};
      const s=account.antitext[jid];
      const type=String(args[0]||'').toLowerCase(); const action=String(args[1]||'').toLowerCase();

      if(type==='delete'&&action==='on'){ s.mode='delete'; return sock.sendMessage(jid,{text:'🚫 *ANTITEXT ENABLED*\n🗑️ Text messages will be deleted.'},{quoted:msg}); }
      if(type==='warn'&&action==='on'){ s.mode='warn'; return sock.sendMessage(jid,{text:'⚠️ *ANTITEXT WARN ENABLED*\n⚠️ Will delete + warn, no kick.'},{quoted:msg}); }
      if(type==='off'||action==='off'){ s.mode='off'; return sock.sendMessage(jid,{text:'✅ *ANTITEXT DISABLED*'},{quoted:msg}); }
      if(type==='on'){ s.mode='delete'; return sock.sendMessage(jid,{text:'🚫 *ANTITEXT ENABLED*\n🗑️ Text messages will be deleted.'},{quoted:msg}); }

      return sock.sendMessage(jid,{text:'📝 *ANTITEXT*\n\n`.antitext delete on` → just delete\n`.antitext warn on` → delete + warn (no kick)\n`.antitext off` → disable'},{quoted:msg});
